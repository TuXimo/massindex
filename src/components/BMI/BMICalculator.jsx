...
export default function BMICalculator({ weight, height, setWeight, setHeight, unit = 'metric', ranges }) {
  ...
  const handleWeightCommit = () => commitInput(localWeight, setWeight, weightMin, weightMax, 'weight');
  const handleHeightCommit = () => commitInput(localHeight, setHeight, heightMin, heightMax, 'height');

  return (
    <div className="p-6 bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-xl flex-1 h-full transition-all duration-300 hover:shadow-2xl hover:border-slate-600 hover:bg-slate-800/60 relative">
