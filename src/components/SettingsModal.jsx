...
export default function SettingsModal({ isOpen, onClose }) {
  ...
  const { userConfig, updateConfig, themeColor } = useConfig();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div className="relative bg-[#050a09] border border-slate-700/50 rounded-2xl w-full max-w-md p-6 shadow-2xl shadow-blue-500/10 overflow-hidden transform transition-all">
        {/* Decorative Glow */}
        ...
        <div className="space-y-6 relative z-10">
            <div>
                ...
                <div className="grid grid-cols-2 gap-2 bg-bmi-input/50 p-1 rounded-xl border border-slate-800">
                    <button 
            ...
            <div className={`space-y-4 transition-all duration-300 ${tempConfig.mode === 'child' ? 'opacity-100 max-h-96' : 'opacity-50 max-h-0 overflow-hidden grayscale pointer-events-none'}`}>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full bg-bmi-input border border-slate-700 rounded-lg p-3 text-white font-bold text-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder-slate-700 h-[54px]"
                            placeholder="10"
                        />
                    </div>
                    <div>
                        <label className="block text-xs font-bold uppercase text-slate-400 mb-2">{t('settings.biologicalSex')}</label>
                        <div className="flex bg-bmi-input border border-slate-700 rounded-lg p-1 h-[54px] gap-1">
                            <button 
                 ...
                 <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
                     ...
                     <p className="text-[11px] text-yellow-200/80 leading-relaxed">
                         <strong className="text-yellow-400">{t('settings.note')}</strong> {t('settings.noteDesc')}
                     </p>
                 </div>
            </div>
        </div>
